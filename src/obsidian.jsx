  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <span style={{ opacity: 0.55, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>Search music, artists, rooms…</span>
        <span style={{
...
function ObsidianLibrary({ section, setSection, tk }) {
  const items = useMemo(() => {
    switch (section) {
      case 'listen-now': return DATA.recents.slice(0, 4).map((r) => ({ id: r.id, title: r.title, subtitle: r.subtitle, art: r.art }));
      case 'playlists':  return DATA.playlists.map((p) => ({ id: p.id, title: p.title, subtitle: `${p.count} songs`, art: p.art }));
      case 'albums':     return DATA.albums.map((a) => ({ id: a.id, title: a.title, subtitle: a.artist, art: a.art }));
      case 'recent':     return DATA.recents.map((r) => ({ id: r.id, title: r.title, subtitle: r.subtitle, art: r.art }));
      case 'radio':      return DATA.stations.map((st) => ({ id: st.id, title: st.title, subtitle: 'Station', art: st.art }));
      default:           return DATA.albums.slice(0, 6).map((a) => ({ id: a.id, title: a.title, subtitle: a.artist, art: a.art }));
    }
  }, [section]);
      ...
      <div style={{ overflowY: 'auto', flex: 1, padding: '6px 14px 14px' }}>
        {items.map((it) => (
          <button key={it.id} onClick={() => {
            const t = DATA.tracks.find((tt) => tt.id === it.id);
            if (t) SonosActions.playTrack(t.id);
            else SonosActions.playTrack(DATA.tracks[Math.floor(Math.random() * DATA.tracks.length)].id);
          }} style={{
            display: 'flex', alignItems: 'center', gap: 12,
